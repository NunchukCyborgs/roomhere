...
@Component({
  ...
  template: `
    <div class="row">
      <div [attr.hidden]="showMore || isEditing" *ngFor="let amen of featuredAmenities" class="column small-6">
        <span><i class="large {{amen.icon}}"></i> {{amen.name}}</span>
      </div>

      <div class="column small-6" [attr.hidden]="showMore"><a (click)="showMore = true">more +</a></div>

      <div [attr.hidden]="!showMore && !isEditing" *ngFor="let amen of amenities" class="column small-6">
